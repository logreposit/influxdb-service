...
public class GenericLogdataBatchPointsFactoryTests
{
    ...
    public void testCreateBatchPoints() throws GenericLogdataBatchPointsFactoryException
    {
        ...
        var batchPoints = this.genericLogdataBatchPointsFactory.createBatchPoints(deviceId, List.of(reading));
        ...
        assertThat(batchPoints.getPoints().get(0).toString())
                .isEqualTo(String.format("Point [name=data, time=%d, tags={key=value}, precision=MILLISECONDS, fields={voltage=24528}]", reading.getDate().toEpochMilli()));
    }
}
