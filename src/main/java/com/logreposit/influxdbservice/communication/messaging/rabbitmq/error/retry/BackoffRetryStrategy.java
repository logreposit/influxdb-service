...
public class BackoffRetryStrategy implements RetryStrategy {
  ...
  public String getExchangeName(long errorCount, String defaultExchangeName) {
    if (errorCount <= 5) return getExchangeNameForRetryInterval(RETRY_INTERVALS[0]);

    if (errorCount <= 10) return getExchangeNameForRetryInterval(RETRY_INTERVALS[1]);

    if (errorCount <= 15) return getExchangeNameForRetryInterval(RETRY_INTERVALS[2]);
