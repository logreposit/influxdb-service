...
public class MessageErrorHandler {
  ...
  @Autowired
  public MessageErrorHandler(RabbitMessageSender messageSender, RetryStrategy retryStrategy) {
  ...
  private static Long getMessageErrorCount(Message amqpMessage) {
    ...
    if (errorCountAsObject != null) errorCount = (Long) errorCountAsObject;
  ...
  public void handleError(Message amqpMessage, Exception exception) {
    ...
    if (exception instanceof NotRetryableMessagingException
        || isNotRetryablePerDefinition(exception)) {
    ...
    logger.info(
        "Exception caught is retryable. Publishing Message to exchange '{}'.", exchangeName);
  ...
  private void publishToExchange(
      String exchangeName, String routingKey, Message amqpMessage, Map<String, Object> headers) {
